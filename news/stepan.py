...
class stepan(object):
    ...
    def crawler_news(self):
        try:
            ...
            news_data = soup.find_all('div', {'class': "accordiongroup aem-GridColumn aem-GridColumn--default--12"})
            if news_data:
                for news in news_data:
                    ...
                    url = "https://www.stepan.com"+str(url_data['href']) if url_data else ''
                    ...
                    description_data = url_soup.find('div',{'class':"text aem-GridColumn aem-GridColumn--default--12"})
                    ...
                    news_dict.update(
                        {"title": title, "news_title_uid": hashlib.md5(title.encode()).hexdigest(),
                         ...
                         "company_id": "stepan", "ticker": "stepan_scrapped", "industry_name": "stepan",
                         "news_provider": "stepan"})
...
url = "https://www.stepan.com/content/stepan-dot-com/en/news-events/news---events.html"
news_obj = stepan(url,logger=logger)
...
isInserted,rowCount = Helper.processNews(news_collection,processed_collection,'stepan')
print('Total rows added Process collection => ' + str(rowCount))

# UPDATING LOG COLLECTION
if (isInserted):
    Helper.makeLog(news_log_collection,processed_collection,'stepan')