    ...
    def crawler_news(self):
        ...
        try:
            ...
            while loop:
                ...
                news_data = soup.find('div', {'class': "news-feed"})
                if news_data:
                    for news in news_data.find_all('div','col-md-2 col-sm-3 news-date')[:1]:
                        try:
                            ...
                            url = "https://www.chitec.com/"+str(url_data['href']) if url_data else ''
                            ...
                            publish_date = Helper.parse_date(news.text)

                            url_response = crawler.MakeRequest(url, 'Get', postData=self.body, headers=self.headers)
                            url_soup = BeautifulSoup(url_response.content, 'html.parser')
                            description_data = url_soup.find('div',{'class':'panel-body'})
                            ...
                            news_dict.update(
                                {"title": title, "news_title_uid": hashlib.md5(title.encode()).hexdigest(),
                                 ...
                                 "company_id": "chitec", "ticker": "chitec_scrapped", "industry_name": "chitec",
                                 "news_provider": "chitec"})
...
url = "https://www.chitec.com/english/news?page={page}"
news_obj = chitec(url,logger=logger)
...
isInserted,rowCount = Helper.processNews(news_collection,processed_collection,'chitec')
print('Total rows added Process collection => ' + str(rowCount))

# UPDATING LOG COLLECTION
if (isInserted):
    Helper.makeLog(news_log_collection,processed_collection,'chitec')